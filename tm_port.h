...
#define TM_MAX_CSIZE    (40)      //max channel num //used if INT8 mdl  //cost TM_MAX_CSIZE*4 Byte
#define TM_MAX_KSIZE    (3*3)       //max kernel_size   //cost TM_MAX_KSIZE*4 Byte
#define TM_MAX_KCSIZE   (3*3*40)   //max kernel_size*channels //cost TM_MAX_KSIZE*sizeof(mtype_t) Byte
...
#define VLOCLK_FREQUENCY 10000UL  // VLOCLK 10 kHz
extern volatile uint32_t timer_overflows;

static inline uint32_t TM_GET_MS() {
    uint32_t current_count = TA0R;
    uint64_t total_counts = ((uint64_t)timer_overflows << 16) + current_count;
    return (uint32_t)((total_counts * 1000ULL) / VLOCLK_FREQUENCY);
}

#define TM_DBGT_INIT()     uint32_t _start, _finish; float _time; \
